...
#' 
#' This function takes analyze.multi output and an optional array of names for those
...
discordance.n <- function(x, setnames=NA, min.freq=0){ # x is a list generated by analyze multi ??-TODO allow chains, burnin and window.size to be passed-??
  # set initial values
  sets <- length(x)-1
  ...
  for(i in 1:slide.wins){
    ...
    win.out <- list()
    discordance <- discordance.mat(winTable, min.freq)
    win.out <- list("window.table" = winTable, "discordance.dist" = discordance, "discordance.tree" = as.phylo(hclust(discordance)), "discordance.min.freq" = min.freq)
    output[[i]] <- win.out
