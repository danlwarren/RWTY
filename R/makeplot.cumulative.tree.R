...
#'
#' @return cumulative.tree Either a single ggplot2 object or a list of ggplot2 objects.
#'
...
makeplot.cumulative.tree <- function(chains, burnin = 0, window.size = 20, facet = TRUE, rank = 'wcsf', p = 0.5){ 
  
  print(sprintf("Creating cumulative split convergence tree"))
  if(rank == 'wcsf'){ 
    RANK = "WCSF"
  }else if(rank == 'sd'){ 
    RANK = "StDev"
