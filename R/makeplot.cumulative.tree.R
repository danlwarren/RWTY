...
#' @param tree A phylo object containing the tree that convergence metric values will be attached to.  If no tree is provided, a p = 0.5 consensus tree will be used.
...
makeplot.cumulative.tree <- function(chains, tree = NA, burnin = 0, window.size = 20, rank = 'wcsf', p = 0.5){ 
  ...
  if(all(is.na(tree))){
    ...
    stop("Convergence tree is not of class phylo")
  }
