...
vignette: >
  %\VignetteIndexEntry{rwty::plot-comparisons}
...
As a general rule of thumb, it's a good idea to have an ESS or pseud-ESS of at least 200.  As such we can be pretty happy about our salamander data, but not so much when it comes to the fungus data:
...
The first plot is a matrix of pairwise comparisons of split frequencies across chains.  Below the diagonal, we have scatter plots with support values for each clade from one chain given on the X axis, with support values from the other chain given on the Y axis.  The R values for a linear regression are given above the diagonal, as are the average standard deviation of split frequencies across that pair of chains.  
